...
def gr_body(request, manager : Actioner.Manager.Manager, projecter : Projecter, project_params) -> None:
    ...
    with gr.Blocks(theme=seafoam) as demo:
        ...
        if manager.getParam("mode") == "base":
            ...
            with gr.Row():
                related_parent_actioners_rad = gr.Radio(label="Related Parent Actioners")
            with gr.Row():
                related_sub_actioners_rad = gr.Radio(label="Related sub Actioners")
                    ...
                    with gr.Column(scale=3):
                        ...
                        with gr.Accordion("Additional",open=False):
                            ...
                            with gr.Row():
                                ...
                                updateall_msgdiff_btn = gr.Button('(N)UAT=>msg_diff')
                            with gr.Row():
     ...
     # with gr.Row():
            ...
            with gr.Tab('ExtProject'):
                    ...
                    with gr.Tab('Ext Tree Managment', visible=False):
                        UI.exttreemanagerManipulator(projecter)

                    with gr.Tab("Others", visible=False):
            ...
            with gr.Accordion(label='Save and load project',open=False):
            # with gr.Row() as r:
            ...
            related_parent_actioners_rad.input(fn=projecter.selectActionerByPath, inputs=[related_parent_actioners_rad], outputs=tree_outlist)
            related_sub_actioners_rad.input(fn=projecter.selectActionerByPath, inputs=[related_sub_actioners_rad], outputs=tree_outlist)
            ...
            freezetask_btn.click(fn=projecter.freezeCurrentTask, outputs=std_output_list)
            gettaskupdationinfo_btn.click(fn=projecter.forceGetCurrentTaskUpdationInfo, outputs=std_output_list)
